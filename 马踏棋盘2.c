...
int around[9][2]={ {0,0}, {-1, 2}, {-1, -2}, {-2, -1}, {-2, 1}, {1, 2}, {1, -2}, {2, 1}, {2, -1}};  //保存的八个方向
        ...
        for(i=0; i<N; i++){     
            for(j=0; j<N; j++){
                    if(path[i][j] == flag){     //flag为1开始递增的数，正好用flag找到，path中马跳转的顺序
                        ...
                        for(i=0; i<N; i++){ 
                            for(j=0; j<N; j++){
                                if(a[i][j] >0 ){                            //因为开始全部初始化未-1，所以将大于0的数正常输出
                                    if(a[i][j] == flag){
                                        printf("\033[32m%4d\033[0m",a[i][j]);   //带颜色的输出数组
                                    }
                                    else{
                        ...
                        usleep(300000);         //在每多输出一个之后，停止0.1s
                    }
                }
            }
        }
